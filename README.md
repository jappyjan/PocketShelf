...
Signing out returns to the login form. The main menu is hidden while logged out. There is no on-screen Close button on the main menu; use the physical Back button to exit from the main menu or login form. Inner pages have a back arrow at the top left, while the signed-in identity stays at the top right.

Search defaults to all formats. Settings cycles through all formats supported by Era Color: EPUB, PDF, MOBI, AZW, AZW3, PRC, FB2, FB2.ZIP, DJVU, DOC, DOCX, RTF, TXT, HTM/HTML, CHM, CBR/CBZ, ACSM, plus its image and audio formats. DRM files still require the reader’s normal authorization. Swipe vertically to scroll search results, popular books, and downloads. Physical next/previous buttons scroll those lists and change description/info pages. Back returns to the previous screen and preserves the browsing position. Searches warm several API pages and fetch more in the background with two screenfuls of runway. Popular books retains the complete collection returned by the server (currently 100 books), rather than treating it as paginated search. Popular, search, and downloads share the same scrolling list component, including native drag events and both primary and secondary hardware page keys. New results append without moving existing rows or showing an intermediate loading screen; a failed request leaves the list usable and offers Retry in the header. Foreground network operations retain their Cancel button.
...
The tests exercise JSON shape changes, server errors, unsafe identifiers, credential/form escaping, cancellation callbacks, size limits, file signatures, repeated downloads, and cleanup after invalid/failed downloads. The redirect tests run against a temporary local HTTPS server with a test CA, preserving certificate verification. Tests also check origin isolation, cookie handshakes, redirect loops, login-only navigation, and error colors. Scrolling tests cover multi-page prefetch, stable offsets, offscreen append refresh suppression, swipe handling, retry, end-of-results, and stale search completion. They do not use a real account or assert hardware compatibility.

Source layout: `src/main.c` only starts the app. `src/app/` handles navigation and background work; `src/ui/` owns screens and reusable widgets; `src/library/` implements the library protocol; `src/net/` handles HTTPS and transfers; `src/storage/` owns persistence and local-file validation; `src/platform/` adapts InkView callbacks and native Library handoff; `src/core/` contains shared models and text/book helpers. See [architecture and module ownership](docs/architecture.md).

Tests mirror those folders and link against module headers, with separate InkView and library fixtures in `tests/support/`. `scripts/sources.sh` is the shared build manifest; `scripts/test-ui.sh` runs the UI regressions alone. `.clang-format` records the C formatting style. `vendor/` contains cJSON 1.7.19 under its MIT license. Protocol/source references and research are in [docs/research.md](docs/research.md).
...
Book descriptions preserve paragraphs and decode HTML entities. Book info shows supplied bibliographic fields (including year/date, publisher, edition, volume, series, pages and ISBN), without inventing missing values. Search requests ask for four results at a time, but every returned book is retained (up to 256 per response; larger responses report an error instead of silently dropping books). The scrolling browser combines pages into one list with fixed-height cover rows and keeps compact summaries in memory. Covers load independently near the viewport so a large collection becomes scrollable before all its images finish. Scrolling uses partial display updates, and appends below the viewport do not refresh the display. A live saved-session metadata and cover request is tested without requesting a book download.
